var ZkeKdfArgon2 = (() => {
  ...
  // zke-argon2-browser-entry.mjs
  var zke_argon2_browser_entry_exports = {};
  __export(zke_argon2_browser_entry_exports, {
  ...
  var ARGON2_ITERATIONS = 2;
  var ARGON2_PARALLELISM = 1;
  var ARGON2_HASH_LEN = 32;
  async function deriveArgon2idRaw(password, salt, opts = {}) {
    const memory = opts.memory ?? ARGON2_MEMORY_KIB;
    const iterations = opts.iterations ?? ARGON2_ITERATIONS;
    const parallelism = opts.parallelism ?? ARGON2_PARALLELISM;
    const pass = typeof password === "string" ? password : new TextDecoder().decode(password);
  ...
  // zke-argon2-browser-entry.mjs
  var ready = deriveArgon2idRaw("warmup", new Uint8Array(16)).catch(() => null);
  async function deriveRaw(password, salt, opts) {
    await ready;
    return deriveArgon2idRaw(password, salt, opts);
  }
  return __toCommonJS(zke_argon2_browser_entry_exports);
})();
